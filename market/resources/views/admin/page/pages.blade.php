                    ...
                    <table class="table table-striped table-hover m-b-0">
                        ...
                        <tr>
                            <th scope="row">
                                ...
                                <a data-fancybox="del" href="{{asset($page->image_name)}}"><img src="{{asset($page->image_name_resize)}}" style="width: 40px;border-radius: 4px;"></a>
                                @endif
                            </th>
                            <td>{{$page->title}}</td>
                            <td>
                            ...
                            <td>
                            <a href="{{route('admin.page',['locale'=>app()->getLocale(),'id'=>$page->id])}}"><button type="button" class="btn btn-secondary"><i class="ti-pencil-alt"></i></button></a>
                            <a onclick="return confirm('{{__('Are you sure?')}}')" href="{{route('admin.page-delete',['locale'=>app()->getLocale(),'id'=>$page->id])}}"><button type="button" class="btn btn-danger"><i class="ti-trash"></i></button></a>
                            </td>
                        </tr>
                        @endforeach
                        </tbody>
                    </table>
                    <?php echo e($pages->appends($_GET)->links()); ?>
                </div>
            </div>
        </div>
    </div>
