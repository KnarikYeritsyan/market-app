...
@section('breadcrumb')
    <ol class="breadcrumb mb-2">
        ...
        <li class="breadcrumb-item"><a href="{{route('admin.setting',app()->getLocale())}}">{{__('Settings')}}</a></li>
    </ol>
@endsection
@section('css')
@endsection
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="" enctype="multipart/form-data">
                        ...
                        @foreach($settings as $setting)
                            @if($setting->name == 'logo' || $setting->name == 'favicon' || $setting->name == 'google_analytics_tracking_id')
                            <div class="form-group row">
                                <label for="{{$setting->name}}" class="col-sm-2 col-form-label">{{$setting->display_name}}</label>
                                <div class="col-sm-10">
                                    @if($setting->name == 'google_analytics_tracking_id')
                                        <input id="{{$setting->name}}" name="{{$setting->name}}" type="{{$setting->type}}" class="form-control" value="{{$setting->value}}">
                                    @else
                                        <input id="{{$setting->name}}" name="{{$setting->name}}" type="{{$setting->type}}" class="form-control" accept="image/*">
                                    @endif
                                </div>
                            </div>
                                @if($setting->name == 'logo' || $setting->name == 'favicon')
                                    <div class="form-group">
                                        @if($setting->value != '')
                                            <img width="100px" src="{{$setting->value}}" alt="{{$setting->display_name}}">
                                        @endif
                                    </div>
                                @endif
                            @endif
